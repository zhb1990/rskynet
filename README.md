...
```
[     0.01] [:00000006] 1000 个来回耗时 6.76ms，平均单程 3.38µs
[     0.32] [:00000006] 三个请求的完成顺序是 ["睡10厘秒", "睡20厘秒", "睡30厘秒"]，总耗时 322ms
```

第二行是重点：三个请求分别要睡 300/100/200 毫秒，总耗时约 300ms 而不是 600ms，说明它们在对端是**并发**处理的；完成顺序也按实际睡眠时长排列，而不是发出顺序。
